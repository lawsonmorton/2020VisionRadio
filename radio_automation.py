...
# INITIALIZE GLOBAL SCRIPT CONTENT
script_content = None
...
if GEMINI_API_KEY:
  try:
    ...
    # Dynamically fetch available models from API client
    available_models = []
    try:
      ...
      print(f"Could not dynamically list Gemini models: {list_err}")
    ...
    for g_model in available_models:
      try:
        ...
        try:
          # Attempt 2: Direct generation without tools
          response = client.models.generate_content(
          ...
          print(f"Gemini {g_model} failed: {g_err1}")
