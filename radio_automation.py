...
# RSS Feed URLs for Facebook Pages (Convert Facebook pages using RSS.app, FetchRSS, or FB2RSS)
SKN_POLICE_FB_RSS = os.environ.get(
    "SKN_POLICE_FB_RSS", ""
).strip()  # E.g. RSS feed for St. Kitts Nevis Police Force
...
# --- 2. FETCH FACEBOOK & OFFICIAL RSS FEEDS ---
fb_context_items = []
rss_sources = [SKN_POLICE_FB_RSS]

for rss_url in rss_sources:
  if rss_url:
    try:
      print(f"Fetching Facebook RSS feed: {rss_url}")
      ...
      for entry in feed.entries[:5]:  # Get latest 5 posts
        ...
        fb_context_items.append(
            f"- [SKN Police Post] {title}: {clean_summary} (Source: {link})"
        )
    except Exception as rss_err:
      print(f"Notice: Could not parse RSS feed ({rss_url}): {rss_err}")

fb_context_str = ""
if fb_context_items:
  fb_context_str = (
      "\nLATEST OFFICIAL SOCIAL MEDIA & POLICE BULLETINS (DIRECT"
      " FEEDS):\n"
      + "\n".join(fb_context_items)
      + "\n"
  )
...
# --- 4. SYSTEM PROMPT WITH WEEKEND OUTLOOK & FB DATA ---
system_prompt = (
    ...
    " outlook.\n   * SPECIAL WEATHER RULE: Right after the Friday weather"
    " forecast section, you MUST create a dedicated section titled 'Weekend"
    " Outlook' that gives a detailed forecast specifically for Saturday and"
    ...
    " STRUCTURE:\n=== SCRIPT 1: NEWS SEGMENT (30 MINUTES) ===\n=== SCRIPT 2:"
    " WEATHER SEGMENT (7 MINUTES) ===\n=== SCRIPT 3: SPORTS SEGMENT (15 MINUTES)"
