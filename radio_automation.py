...
# 1. VERIFY ENVIRONMENT VARIABLES BEFORE EXECUTING
required_secrets = [
    "GEMINI_API_KEY",
    "SENDER_EMAIL",
    "SENDER_PASSWORD",
...
try:
  ...
  print("Script generated successfully from Gemini API.")
  ...
  # Parse line by line to style stage cues and source links
        ...
        if "]" in part:
          ...
          if cue_text.startswith("Source:"):
            ...
            r_cue.font.color.rgb = RGBColor(
                0x55, 0x55, 0x55
            )  # Grey for presenter source links
          else:
            r_cue.font.color.rgb = RGBColor(
                0xC0, 0x39, 0x2B
  ...
  print(f"Document created and saved locally: {doc_filename}")

  # 3. EMAIL ATTACHMENT VIA SMTP
  msg = MIMEMultipart()
...
"""
  ...
  print(f"Connecting to SMTP server to send email to {PRESENTER_EMAIL}...")
